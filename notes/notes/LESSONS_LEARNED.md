...
## 10. Supabase + Prisma (`@prisma/adapter-pg`) SSL 인증서 오류

### 문제
notion-form-limiter 앱에서 폼 생성 버튼을 눌러도 아무 동작도 하지 않음. 에러 핸들링 catch가 없어서 무반응처럼 보였고, catch를 추가하니 "Notion 통신 중 오류" 메시지가 표시됨.

### 원인
`lib/prisma.ts`의 `pg.Pool` 설정에 SSL 옵션이 없었음. Supabase는 자체 서명 인증서(self-signed certificate)를 사용하는데, `pg` 라이브러리는 기본적으로 인증서 체인을 검증하려 함 → `Error opening a TLS connection: self-signed certificate in certificate chain` 발생.

로컬(`.env.local`) 개발 환경에서는 동일한 에러가 발생하지 않았기 때문에 배포 후에야 발견됨.

### 삽질 과정
1. 에러 메시지가 generic("Notion 통신 중 오류")해서 Notion API 문제로 오인
2. `prisma db push` 실행 → "DB 스키마 동기화 완료" → DB 연결 자체는 되는 것처럼 보임 (push는 성공)
3. `DB_ERROR` 코드로 분리한 뒤 실제 에러 메시지를 응답에 노출시켜 원인 파악
...
### 교훈
- **Supabase + `pg` Pool 사용 시 `ssl: { rejectUnauthorized: false }` 는 필수.**
- `prisma db push`는 성공해도 런타임 Pool 연결은 별개로 실패할 수 있다 — 스키마 동기화와 런타임 연결을 혼동하지 마라.
- **API 에러는 generic 메시지로 뭉뚱그리지 마라.** `NOTION_ERROR`, `DB_ERROR` 등으로 구분하고, 디버그 시에는 실제 에러 메시지를 응답에 포함시킬 수 있는 임시 코드를 준비해두면 원인 파악이 훨씬 빠르다.

---

## 11. Electron 앱 Windows 패키징 삽질 (wee-log)

### 문제
`npm run package` 실행 시 세 가지 에러가 순차적으로 발생하며 `.exe` 인스톨러 생성 실패.

### 원인 및 해결 과정

**1단계: Visual Studio 없음**
```
Error: Could not find any Visual Studio installation to use
```
- 원인: `better-sqlite3`가 C++ 네이티브 모듈이라 패키징 시 electron-builder가 Electron 버전에 맞춰 재컴파일(`npmRebuild: true`)을 시도하는데, C++ 컴파일러(MSVC)가 없었음
- 해결: `winget install Microsoft.VisualStudio.2022.BuildTools` → VS Installer에서 **"C++를 사용한 데스크톱 개발"** 워크로드 선택 후 설치
- 단, dev 모드에서 이미 Electron 호환 prebuilt 바이너리가 있었으므로 `electron-builder.yml`에서 `npmRebuild: false`로 설정해 재빌드 자체를 스킵하는 것도 가능

**2단계: 코드 서명 도구 심볼릭 링크 권한 없음**
```
ERROR: Cannot create symbolic link : 클라이언트에 필요한 권한을 보유하고 있지 않습니다.
```
- 원인: winCodeSign(코드 서명 도구) 압축 해제 시 macOS용 심볼릭 링크를 생성하려 하는데, Windows는 기본적으로 일반 사용자에게 심볼릭 링크 생성 권한이 없음
- `CSC_IDENTITY_AUTO_DISCOVERY=false`, `sign: null` 설정으로도 winCodeSign 다운로드 자체는 막히지 않음
- 해결: **PowerShell을 관리자 권한으로 실행** 후 패키징
...
### 교훈
- **Electron 앱 첫 패키징은 개발 환경보다 훨씬 많은 도구가 필요하다.** dev 모드가 된다고 패키징이 바로 되는 게 아님.
- **better-sqlite3 같은 네이티브 모듈이 있으면 `npmRebuild: false` + prebuilt 바이너리 방식이 편하다.** VS Build Tools 없이도 패키징 가능.
- **코드 서명 없이 패키징할 때도 winCodeSign은 다운로드된다.** 심볼릭 링크 권한 문제가 생기면 관리자 PowerShell로 해결.
- **MVP 단계에서 코드 서명 인증서(연 100~200만원)는 불필요.** 설치 시 "알 수 없는 게시자" 경고가 뜨지만 추가 정보 → 실행으로 설치 가능. 정식 계약 후 사측에서 처리.
