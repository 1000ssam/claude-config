...
- **한마디**: 사람이 읽는 코드 → 컴퓨터가 실행할 수 있는 코드로 번역하는 과정
- **wee-log 예시**: TypeScript(.ts) + React JSX(.tsx) → JavaScript(.js). 브라우저/Node는 TypeScript 문법을 직접 못 읽기 때문에 번역 필요
- **번들링(Bundling)**: 컴파일과 함께 여러 파일을 하나(또는 소수)로 합치는 작업. electron-vite가 둘 다 담당
- **빌드(Build)**: 컴파일+번들링을 포함한 전체 변환 과정을 통칭하는 말. `npm run build` → `out/` 폴더에 결과물
- **패키징(Packaging)**: 빌드 결과물을 설치 파일(.exe)로 묶는 추가 단계. `npm run package` → `dist/` 폴더에 인스톨러 생성
- **비유**: 한국어 레시피(TypeScript) → 일본어 번역(컴파일) → 일본 요리사(Node/브라우저)가 실행 가능
- **"Script"인데 왜 즉시 실행이 안 되나?**: JavaScript는 브라우저가 JS 엔진을 내장해서 바로 실행 가능. TypeScript는 JS에 타입 문법을 얹은 것인데 브라우저/Node가 타입 문법을 모름. 컴파일 시 타입 정보는 흔적 없이 제거되고 순수 JS만 남음. 즉, 타입은 코딩할 때 오류를 잡아주는 도구일 뿐, 실행 시점에는 존재하지 않음
- **그럼 왜 JS 대신 TS를 쓰나?**: JS는 실행해봐야 오류를 알 수 있음. TS는 코딩 중 에디터가 바로 빨간줄로 알려줌 + 자동완성이 강력해짐. 프로젝트가 클수록, 암호화/DB처럼 민감한 로직일수록 TS가 안전망 역할. 작은 스크립트나 빠른 프로토타입은 JS가 나을 수도 있음 — 트레이드오프
- **AI 바이브코딩 시 TS 장점은 희석되나?**: 어느 정도 그렇다. 오류를 코딩 중 발견하는 게 TS 핵심인데, AI가 통째로 생성하면 어차피 실행해서 확인하는 흐름. 그러나 타입이 AI한테 문맥 역할을 해서 구조를 정확히 파악하고 일관된 코드를 생성하는 데 도움. 결론: 혼자 손코딩 시 장점 100%, AI 바이브코딩 시 50% 정도지만 단점도 없으니 기존 TS 프로젝트는 유지가 맞음

| 명령어 | 하는 일 | 결과물 |
|--------|---------|--------|
| `npm run dev` | 개발 서버 실행 (번역 생략, 즉시 반영) | 화면만 |
| `npm run build` | 컴파일만 | `out/` |
| `npm run package` | 컴파일 + .exe 인스톨러 생성 | `out/` + `dist/` |
