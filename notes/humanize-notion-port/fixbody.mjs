...
await notion.updatePageMarkdown(id, body);
console.log('본문 재작성 완료. 파일명 링크화 방지 검증:');
const md = await notion.getPageMarkdown(id);
const bogus = (md.match(/http:\/\/[a-z0-9_.*-]+\.(md|py)/gi) || []);
console.log('남은 가짜 자동링크 수:', bogus.length, bogus.slice(0,5).join(' '));
